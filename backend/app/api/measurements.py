...
from ..domain.constants import (
    DATA_SOURCE_LABELS,
    MANUAL_DATA_SOURCE_CHOICES,
    PERIOD_LABELS,
    completeness_label,
)
from ..services import aggregation_service, measurement_service, query_service, station_service
...
def create_entries():
    ...
    data_source = validator.choice("data_source", "数据来源",
                                   choices=MANUAL_DATA_SOURCE_CHOICES,
...
@bp.post("/aggregate-daily")
def aggregate_daily():
    """按日汇总: 把指定日期的小时均值汇总为日均值, 可用于历史数据补算."""
    data = json_payload()
    validator = Validator(data)
    station_id = validator.number("station_id", "监测点", required=False, minimum=1)
    day = validator.date_field("date", "汇总日期", required=True)
    validator.raise_if_invalid()
    return aggregation_service.aggregate_for_date(
        day, station_id=int(station_id) if station_id else None
    )
...
def entry_context():
    ...
    return {
        ...
        "data_sources": [
            {"value": key, "label": DATA_SOURCE_LABELS[key]} for key in MANUAL_DATA_SOURCE_CHOICES
        ],
